...
In a basketball tournament, we could always compare any two teams, either A ≼ B of B ≼ A due to the rule that every team plays every other team exaclty once. This relation satisfies the *Totality* property. Thus, ≼ is a total order as well.
...
Is it a *total order*?
To be a total order it would have to compare every pair; for any two points (x1, y1) and (x2, y2) either (x1, y1) ≼ (x2, y2) or (x2, y2) ≼ (x1, y1). That fails, since:
...
So B !≼ A. Thus A and B are incomparable, so the order is not total.
...
*** Problem 9
*Greatest:* the champion (the team ranked 1st).

*Least:* the last-placed team.

*Maximal elements:* in a total order maximal = greatest, so the single champion.

*Minimal elements:* minimal = least, so the single last-place team.
...
** Solution
- Finding sup A (Supremum)
  *Claim.* sup A = 1.

  *Proof.*
  1. 1 is an upper bound: m/n < 1 (since m/n < n/n = 1) for all m/n ∈ A.
