...
\begin{align*}
P(\text{first match at } j) &= \sum_{j = 1}^{10 - 2 + 1} j \cdot \frac{\binom{10-j}{2-1}}{\binom{10}{2}} \\
&= \frac{11}{3} \\
&\approx 3.666 \\
\end{align*}

Sanity check: $n = 10, k = 2$, predicted $P(\text{first match at } j) \approx 3.666$, empirical $3.667$.
